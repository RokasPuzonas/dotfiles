...
run_once({ "zsh", "unclutter -root" }) -- entries must be separated by commas
...
awful.util.screen_locker = "slock"
...
local mysystemmenu = {
    ...
    { "Edit config", function() awful.spawn.with_shell(("%s %s %s/config.lua"):format(config.terminal, config.editor, config_dir)) end },
...
-- {{{ Key bindings
local clientkeys = joinKeysList(require("keys.client"))

local clientbuttons = joinButtonsList(require("buttons.client"))

-- Set keys
root.keys(joinKeysList(require("keys.global")))
-- }}}
